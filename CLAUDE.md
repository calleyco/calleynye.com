# CLAUDE.md — Calley Nye Portfolio & Calley Co. Site
...
UI Software Engineer role on the Claude.ai consumer product team.
...
- Streaming text content (relevant to Claude.ai patterns Calley is studying) gets
...
Create these as files in `.claude/agents/`. Claude Code will use them as specialized
...
### `.claude/agents/vue-to-react-tutor.md`
...
2. **Streaming and Real-Time UIs** — WebSocket multiplayer at McKinsey,
   directly maps to Claude.ai's hardest UI challenges
...
When Claude Code starts a new session on this project for the first time:

1. Create `.claude/agents/accessibility-auditor.md`
2. Create `.claude/agents/vue-to-react-tutor.md`
3. Create `.claude/agents/performance-auditor.md`
4. Create `.claude/agents/content-voice.md`
